...
const note = new _modules_MyNotes__WEBPACK_IMPORTED_MODULE_5__["default"]();
...
class Like {
  ...
  events() {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(".like-box").on('click', this.our_click_dispatcher.bind(this));
  }
  ...
  create_like(current_like_box) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      ...
      type: 'POST',
      data: {
        'leader_id': current_like_box.data('player')
      },
      success: response => {
...
class MyNotes {
  constructor() {
    if (document.querySelector('#my-notes')) {
      axios__WEBPACK_IMPORTED_MODULE_0__["default"].defaults.headers.common["X-WP-Nonce"] = bridge_data.nonce;
  ...
  async delete_note(e) {
    const thisNote = this.find_nearest_parent_li(e.target);
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0__["default"].delete(bridge_data.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"));
  ...
  async update_note(e) {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0__["default"].post(bridge_data.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"), ourUpdatedPost);
      this.makeNoteReadOnly(thisNote);
      console.log(response);
    } catch (e) {
      console.log("Sorry");
  ...
  async createNote() {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0__["default"].post(bridge_data.root_url + "/wp-json/wp/v2/note/", ourNewPost);
      console.log(response);
...
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var axios__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! axios */ "./node_modules/axios/lib/axios.js");
// Код для организации поиска по сайту на стороне клиента
...
class Search {
  ...
  async getResults() {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0__["default"].get(bridge_data.root_url + "/wp-json/bridge/v1/search?term=" + this.searchField.value);
      const results = response.data;

      // Разбиваем область вывода результатов поиска на три равные колонки
      //  В первой колонке выводим общую информацию соответствующую поиску
      // поиск по блогу и статическим страницам.
      //<div class="one-third"> шириной 30% от общей.
      // во второй информация по играм, правилам, игрокам
      // в третьей по местам проведения и событиям
      ...
      this.resultsDiv.innerHTML = `
        <div class="row">
          ...
          <div class="one-third">
            <h2 class="search-overlay__section-title">Plays</h2>
            ${results.play.length ? '<ul class="link-list min-list">' : `<p>No play match that search. <a href="${bridge_data.root_url}/play">View all play</a></p>`}
              ${results.play.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join("")}
            ${results.play.length ? "</ul>" : ""}

            <h2 class="search-overlay__section-title">Rules</h2>
            ${results.rule.length ? '<ul class="link-list min-list">' : `<p>No rule match that search. <a href="${bridge_data.root_url}/rule">View all rules</a></p>`}
              ${results.rule.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join("")}
            ${results.rule.length ? "</ul>" : ""}

            <h2 class="search-overlay__section-title">Leaders</h2>
            ${results.leader.length ? '<ul class="professor-cards">' : `<p>No leader match that search.</p>`}
              ${results.leader.map(item => `
                ...
                `).join("")}
            ${results.leader.length ? "</ul>" : ""}

          </div>
          <div class="one-third">
            <h2 class="search-overlay__section-title">Locations</h2>
            ${results.location.length ? '<ul class="link-list min-list">' : `<p>No location match that search. <a href="${bridge_data.root_url}/location">View all location</a></p>`}
              ${results.location.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join("")}
            ${results.location.length ? "</ul>" : ""}

            <h2 class="search-overlay__section-title">Events</h2>
            ${results.event.length ? "" : `<p>No events match that search. <a href="${bridge_data.root_url}/events">View all events</a></p>`}
    ...
    document.body.insertAdjacentHTML("beforeend", `
      <div class="search-overlay">
        <div class="search-overlay__top">
          <div class="container">
            <i class="fa fa-search search-overlay__icon " style="font-size: 225%;" aria-hidden="true"></i>
            <input type="text" class="search-term" placeholder="What are you looking for?" id="search-term">
            <i class="fa fa-window-close search-overlay__close" style="font-size: 225%;" aria-hidden="true"></i>
          </div>
        </div>        
        <div class="container">
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkbridge_montenegro_theme"] = self["webpackChunkbridge_montenegro_theme"] || [];
